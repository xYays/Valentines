...
(() => {
  // ===== SETTINGS =====
  const SETTINGS = {
    allowLeftClickMove: false,          // League-style movement on right click
    angelAutoTriggerWhenClose: false,   // set true if you want no-click interaction
    internalW: 320, internalH: 180
  };
  ...
  // pointer event button normalization
  ...
  class ParkScene{
    ...
    updateEvent(dt){
      ...
      if(ev.stage==="approach"){
        ...
        // failsafe (guarantees question even if movement gets stuck)
        if(ev.t > 1.2){
          p.pose="sit";
          ev.stage="sit";
          ev.t=0;
    ...
    update(dt){
      ...
      if(this.event.stage==="idle" && nearAngel){
        ...
        if(SETTINGS.angelAutoTriggerWhenClose) this.beginAngel();
      }
    ...
    onPointerDown(world, button){
      ...
      // interact with Angel (NO hitbox required, only proximity)
    ...
    render(ctx){
      ...
      if(this.event.bubble){
        ...
        ctx.fillText("will you be my valentine?", x+bw/2, y+bh/2+1);
      }
    }
  }
